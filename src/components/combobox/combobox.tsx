...
export function Combobox({
  ...
  const DefaultVariant = ({
    ...
    >
      <span
        className={`${
          isButtonLabel ? 'max-w-[200px] truncate h-full flex items-center' : ''
        }`}
      >
        {item.label}
      </span>
      ...
      <div className='flex flex-col items-start'>
        <div
          className={`text-sm font-medium${
            isButtonLabel
              ? ' max-w-[160px] truncate h-full flex items-center'
              : ''
          }`}
        >
          {item.label}
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <div className='flex flex-col items-start gap-1 w-full'>
        {label ? (
          <label className='text-xs font-semibold text-tertiary-foreground'>
            {label}
          </label>
        ) : null}
