...
function SideBarHeader({ className, children, isOpen, onOpenChange, ...props }: SideBarHeaderProps) {
  return (
    <div data-testid="sidebar-header" className={cn('w-full relative px-3', className)} {...props}>
      {children}
      <div aria-hidden className="pointer-events-none absolute top-[7px] -right-px w-[10px] h-[34px] bg-sidebar-background z-10" />
      <button
        className="absolute top-2 -right-[24px] w-8 h-8 p-2 flex items-center bg-surface rounded-r-full text-icon-tertiary shadow-modal-5 z-0"
