...
const itemVariants = cva(
  'h-[40px] flex items-center no-underline select-none no-focus:bg-list-hover flex-start w-full px-[16px] cursor-pointer text-sm font-normal text-list-label hover:text-primary-foreground hover:bg-list-hover transition duration-300 ease-in-out [&>svg>path]:opacity-[.45] [&>svg]:mr-2'
);
    ...
    return (
      ...
      >
        <div className='animate-slide-left w-full fixed transition-all duration-300 ease-out'>
          ...
          <div className='w-full h-[1px] bg-list-hover my-[8px]'></div>

          {selectedSub &&
            selectedSub[0].props.children.map((item: any) => item)}
        </div>
      </div>
    );
  };
